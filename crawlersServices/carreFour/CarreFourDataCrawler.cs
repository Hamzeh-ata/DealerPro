...
namespace DealerPro.crawlersServices.carreFour
{
    public class CarreFourDataCrawler
    {
        private HashSet<string> storedproducts = new HashSet<string>();
        int scrollCounter = 0;
        private readonly FireBase _firebase;
        public CarreFourDataCrawler()
        {
        }
        public CarreFourDataCrawler(FireBase firebase)
        {
            _firebase = firebase;
        }
        public async Task CrawlerAsync(string url, string endPoint, string storeName)
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                while (flag)
                {
                    var product = wait.Until(driver => driver.FindElements(By.CssSelector(".css-b9nx4o")));
                    ...
                    try
                    {
                        ...
                        for (; i < productsCount; i++)
                        {
                            ...
                            if (wait.Until(driver => product[i].FindElements(By.CssSelector(".css-iqeby6"))).Count > 0)
                            {
                                orginalPriceBeforeClick = wait.Until(driver => product[i].FindElement(By.CssSelector(".css-iqeby6 > .css-17fvam3"))).Text.Replace(",", ""); ;
                                currentPrice = wait.Until(driver => product[i].FindElement(By.CssSelector(".css-2a09gr > .css-17fvam3"))).Text.Replace(",", ""); ;
                            }
                            else
                            {
                                currentPrice = wait.Until(driver => product[i].FindElement(By.CssSelector(".css-fzp91j > .css-17fvam3"))).Text.Replace(",", ""); ;
                            ...
                            string NameBeforeClick = product[i].FindElement(By.CssSelector(".css-11qbfb > .css-1nhiovu > a")).Text;
                            ...
                            if (await _firebase.CheckIfAlreadyExists(endPoint, storeName, NameBeforeClick, orginalPriceBeforeClickWithoutCurrency, currentPriceBeforeClickWithoutCurrency))
                            {
                            ...
                            else
                            {
                                ...
                                driver.ExecuteScript("window.open(arguments[0]);", product[i].FindElement(By.CssSelector(".css-1nhiovu > a")).GetAttribute("href"));
                                // switch to the new tab
                                driver.SwitchTo().Window(driver.WindowHandles.Last());
                                Thread.Sleep(10000);
                                var Name = wait.Until(driver => driver.FindElement(By.CssSelector("main.css-z4qeze > .css-peby8 > .css-1cvrzon > .css-q7lfpb > section.css-168eikn > h1.css-106scfp")));
                                ...
                                Thread.Sleep(5000);
                                await _firebase.InsertDataIntoFirebase(endPoint, storeName, Name.Text, Image.GetAttribute("src"), oldPricePriceClickWithoutCurrency, currentPriceClickWithoutCurrency, socket, brand, liTexts, productUrl);
