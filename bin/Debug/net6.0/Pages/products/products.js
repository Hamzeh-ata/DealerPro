...
const searchQuery = urlParams.get('SearchValue');
const categorySearchQuery = urlParams.get('Category');
const SearchQuery = urlParams.get('ProductName');
...
$(document).ready(function () {
    ...
    // Call the loadNextSet function to fetch and append the first set of items.
    else if (searchQuery && categorySearchQuery) {
        loadItemsBySearch(categorySearchQuery,searchQuery);
        componentsCounter = 0;
        if (storeNameUrl) {
        ...
        loadItemsBySearchUsingName(searchQuery);
        componentsCounter = 0;
        if (storeNameUrl) {
            filterElementsByStoreName(storeNameUrl);
        } else if (!storeNameUrl) {
        ...
        window.location.href = "https://localhost:7277/home/";
    }
    if (storeNameUrl) {
        filterOnPageLoad();
    }
...
function loadItems() {
    $.ajax({
        url: '/api/GetProductByCategory/' + category,
        method: 'GET',
        success: function (data) {
            data.forEach(function (component) {
                ...
                var newComponent = $("<div class='productCol col-lg-2 col-md-4 col-sm-12 m-3' data-storename=" + component.storeName + " data-price=" + component.price + ">  <div class='componentCard h-100 d-flex flex-column product' > <div class='deatils' id='productCard' data-storename=" + component.storeName + " data-id=" + component.productId + " data-category=" + component.category + " > <div class='componentImg'> <img src='" + component.image + " ' class='img-fluid animated-image '></div><h1 class='text-center'>" + component.name + "</h1><div class='prices d-flex justify-content-center'></div><div class='storeName'><span>" + component.storeName + "</span></div></div><div class='compareButton'><a class='compare' id='compareButton' data-id=" + component.productId + "  data-category=" + component.category + " data-storename=" + component.storeName + ">Compare</a></div>  </div> </div></div>");
    ...
    $.ajax({
        ...
        success: function (data) {
            data.forEach(function (component) {
                ...
                for (desc in description) {
                    $("#" + productId).append('<li class="desciptionListItemCompare">' + description[desc] + '</li>')
                }
...
function loadItemsBySearch(category,name) {
    $.ajax({
        url: '/api/HomeSearch/' + category + "/" + name,
        method: 'GET',
        success: function (data) {
            totalComponentsCounter
            if (data.length > 0) {
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            $("#filterSelect").hide();
            $(".selectProduct").hide();
            var responsiveImage = $('<img>').addClass('img-fluid').attr('src', './imgs/NoProductsFound.png');
            responsiveImage.hide();
            // Create a new column element with Bootstrap classes
            var column = $('<div>').addClass('col-6 d-flex justify-content-center').append(responsiveImage);

            // Append the column to the #componentRow element
            $("#componentRow").append(column);
            responsiveImage.fadeIn(1000);
        }
    });

}
function loadItemsBySearchUsingName(name) {
    $.ajax({
        url: '/api/HomeSearch/'+ name,
        method: 'GET',
        success: function (data) {
            totalComponentsCounter
            if (data.length > 0) {
